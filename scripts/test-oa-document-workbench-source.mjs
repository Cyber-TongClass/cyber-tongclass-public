...
test("workbench loads bounded authenticated preview pages without leaking credentials", async () => {
  ...
  assert.match(code, /previewPageCacheRef/)
  assert.match(code, /previewPageRequestsRef/)
  assert.match(code, /prefetchPreviewPages/)
  assert.match(code, /if \(!pageCount \|\| !renderedPage \|\| !prefetchPreviewPages\.length\) return/)
  assert.match(code, /renderedPage/)
  assert.match(code, /OADocumentCanvas page=\{renderedPage\.page\}/)
  assert.doesNotMatch(code, /setPreviewPageUrl\(""\)/)
