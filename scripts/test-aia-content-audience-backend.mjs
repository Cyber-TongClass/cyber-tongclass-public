...
test("institute content queries reuse every eligible real record and resolve linked identities", () => {
  ...
  assert.match(source, /readNewsBucket\(ctx, undefined, limit\)/)
  assert.match(source, /readNewsBucket\(ctx, "tong_class", limit\)/)
  assert.match(source, /readNewsBucket\(ctx, "institute", limit\)/)
  assert.match(source, /\.eq\("siteScope",\s*siteScope\)/)
  assert.match(source, /readPublicationBucket\([\s\S]*?limit:\s*number/)
  assert.match(source, /readNewsBucket\([\s\S]*?limit:\s*number/)
