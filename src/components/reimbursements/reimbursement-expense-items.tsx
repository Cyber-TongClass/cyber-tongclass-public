...
const expenseHeaderClassName =
  "aia-bg-tag sticky top-0 z-20 h-8 border-r aia-border-rule px-2 py-1 text-left align-middle text-xs font-semibold text-[hsl(var(--aia-ink))]"

const expenseCellClassName =
  "min-w-[180px] border-r aia-border-rule p-0 align-middle"
...
export function ReimbursementExpenseItems({
  ...
  return (
    <div className="space-y-3">
      <div className="reimbursement-expense-dense-table overflow-x-auto border aia-border-rule">
        <table className="w-full min-w-[760px] border-collapse text-xs">
          ...
          <tbody>
            {rows.map((row, rowIndex) => (
              <tr key={row.key} className="h-8 border-b aia-border-rule last:border-b-0 hover:bg-[hsl(var(--aia-tag))]">
                <td className="aia-bg-tag sticky left-0 z-10 w-12 border-r aia-border-rule p-0 align-middle">
                  <div className="flex h-8 items-center justify-center gap-1">
                    <span className="aia-text-muted w-4 text-center text-[11px]">{rowIndex + 1}</span>
                    {rows.length > 1 ? (
                      <Button
                        type="button"
                        variant="ghost"
      ...
      <div className="reimbursement-expense-toolbar aia-bg-tag flex flex-col gap-2 border aia-border-rule px-2 py-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap gap-2">
          <Button type="button" variant="outline" size="sm" className="min-h-11 rounded-none px-3 text-xs" onClick={onAddRow}>
            <Plus className="mr-1.5 h-3.5 w-3.5" />
            增加一行
          </Button>
          {onAppendRows ? (
            <Button type="button" variant="outline" size="sm" className="min-h-11 rounded-none px-3 text-xs" onClick={() => {
        ...
        <p className="text-sm font-semibold text-[hsl(var(--aia-ink))] sm:text-base">总计：{totalLabel}</p>
      </div>

      {bulkOpen && onAppendRows ? (
        <div className="aia-bg-tag grid gap-3 border aia-border-rule p-4">
          <div>
            <p className="text-sm font-medium text-[hsl(var(--aia-ink))]">粘贴 CSV 文本后追加到当前明细</p>
            <p className="aia-text-muted mt-1 text-xs">每行格式：项目名称,金额,备注。可包含表头；备注可为空。</p>
          </div>
          <Textarea
            className="min-h-32 bg-transparent font-mono text-sm"
            placeholder={"项目名称,金额,备注\n机票,3500,北京往返\n住宿,1200,会议期间住宿"}
            value={bulkText}
            onChange={(event) => setBulkText(event.target.value)}
          />
          {bulkErrors.length ? (
            <div role="status" className="border-y aia-border-rule px-3 py-2 text-xs text-[hsl(var(--aia-red-deep))]">
