    ...
    <section className="space-y-3">
      ...
      {forms.length === 0 ? (
        ...
        <div className="divide-y rounded-xl border bg-white shadow-sm">
          {forms.map((form) => (
            <Link key={form._id} href={`/tong-class/intranet/forms/${form.slug}`} className="flex flex-col gap-2 px-4 py-3 transition-colors hover:bg-slate-50 sm:flex-row sm:items-center sm:justify-between">
    ...
    <div className="min-h-screen bg-[hsl(211,30%,97%)]">
      <section className="bg-primary relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16 relative">
          ...
          <Link href="/tong-class/intranet" className="inline-flex items-center gap-1 text-sm text-white/60 hover:text-white mb-4 transition-colors"><ArrowLeft className="h-4 w-4" />返回内网</Link>
